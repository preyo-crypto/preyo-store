    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: {
          parts: [
            {
              text: `Generate a high-quality product photo of a t-shirt: ${aiImagePrompt}. The t-shirt should be centered, flat-lay or on a minimal background, high resolution, professional photography style.`,
            },
          ],
        },
        config: {
      ...
      <main>
        {view === 'shop' ? (
          ...
          <div className="max-w-7xl mx-auto px-4 py-8">
            {!isAdminLoggedIn ? (
              <div className="max-w-md mx-auto mt-10 sm:mt-20 p-6 sm:p-8 bg-white rounded-3xl shadow-xl border border-gray-100">
              ...
              <div className="flex flex-col lg:flex-row gap-8">
                {/* Sidebar */}
                <aside className="lg:w-64 flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-x-visible pb-4 lg:pb-0 scrollbar-hide shrink-0">
                  {[
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-3 rounded-xl font-bold text-xs sm:text-sm transition-all border whitespace-nowrap ${adminTab === tab.id ? 'bg-primary text-white shadow-lg border-primary shadow-indigo-100' : 'bg-white text-text-muted hover:bg-bg-base border-border-base'}`}
                    >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-3 rounded-xl font-bold text-xs sm:text-sm bg-white text-red-500 hover:bg-red-50 border border-red-50 transition-all ml-auto lg:ml-0 lg:mt-auto"
                  >
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="col-span-2 space-y-2">
                              ...
                              <div className="flex gap-2">
                                <input 
                                  required
                                  className="flex-grow px-4 py-2.5 bg-white border border-border-base rounded-xl outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-sm"
                                  placeholder="https://example.com/image.jpg"
                                  value={productFormData.imageUrl}
                                  onChange={(e) => setProductFormData({...productFormData, imageUrl: e.target.value})}
                                />
                                {productFormData.imageUrl && (
                                  <div className="w-10 h-10 rounded-lg overflow-hidden border border-border-base shrink-0 bg-gray-50">
                                    <img src={productFormData.imageUrl} alt="Preview" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                                  </div>
                                )}
                              </div>
                              ...
                              <div className="pt-2">
                                <div className="p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100 space-y-3">
                                  <div className="flex items-center gap-2 text-primary">
                                    <Sparkles size={16} />
                                    <span className="text-[10px] font-bold uppercase tracking-wider">AI Image Generator</span>
                                  </div>
                                  <div className="flex gap-2">
                                    <input 
                                      type="text" 
                                      placeholder="Describe the design (e.g., 'Retro cyberpunk cat on black tea')..."
                                      className="flex-grow px-3 py-2 bg-white border border-indigo-100 rounded-lg text-xs outline-none focus:border-primary transition-all"
                                      value={aiImagePrompt}
                                      onChange={(e) => setAiImagePrompt(e.target.value)}
                                      onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), handleGenerateAiImage())}
                                    />
                                    <button
                                      type="button"
                                      ...
                                      className={`px-4 py-2 rounded-lg font-bold text-[10px] uppercase tracking-wider transition-all flex items-center gap-2 ${
                                        isGeneratingImage || !aiImagePrompt.trim()
                                        ? 'bg-indigo-100 text-indigo-300 cursor-not-allowed'
                                        : 'bg-primary text-white hover:bg-primary-hover shadow-sm shadow-indigo-200'
                                      }`}
                                    >
                                      {isGeneratingImage ? (
                                  ...
                                  <p className="text-[9px] text-indigo-400 font-medium">Use AI to create a unique t-shirt mockup instantly.</p>
                                </div>
                              </div>
                            </div>
                            <div className="col-span-2">
        ...
        {isCheckoutOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-2 sm:p-4">
            ...
            <motion.div 
              ...
              className="relative bg-white w-full max-w-4xl rounded-[1.5rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[95vh] md:h-[min(800px,90vh)]"
            >
              <button 
                onClick={() => setIsCheckoutOpen(false)}
                className="absolute top-3 right-3 md:top-6 md:right-6 p-2 bg-white/80 backdrop-blur-md text-gray-500 hover:text-black rounded-full shadow-sm transition-all z-20 border border-border-base"
              >
                <X size={18} />
              </button>

              {/* Order Summary Side */}
              <div className="hidden md:flex md:w-[38%] bg-bg-base p-6 lg:p-10 border-r border-border-base flex-col h-full overflow-y-auto">
                ...
                <div className="pt-6 mt-6 border-t border-border-base">
                  <div className="flex justify-between items-end">
                    ...
                    <p className="text-3xl lg:text-4xl font-black text-primary leading-none">৳{isBuyNowFlow ? selectedProduct?.price : cartTotal}</p>
                  </div>
                </div>
              </div>

              {/* Mobile Order Summary (Collapsible or just simplified) */}
              <div className="md:hidden bg-bg-base p-4 border-b border-border-base">
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="text-base font-bold text-text-base">Your Order</h3>
                    <p className="text-[10px] text-text-muted uppercase font-bold">{isBuyNowFlow ? '1 Item' : `${cart.length} Items`}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xl font-black text-primary">৳{isBuyNowFlow ? selectedProduct?.price : cartTotal}</p>
                  </div>
                </div>
              </div>

              {/* Form Side */}
              <div className="flex-grow p-5 sm:p-8 lg:p-12 overflow-y-auto bg-white">
                <div className="max-w-md mx-auto">
                  <h3 className="text-xl sm:text-2xl font-bold mb-1 text-text-base">Checkout</h3>
                  <p className="text-text-muted text-xs sm:text-sm mb-6 sm:mb-8">Secure your order by providing delivery details.</p>
                  
                  <form onSubmit={placeOrder} className="space-y-4 sm:space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                      <div className="sm:col-span-2">
                        <label className="block text-[10px] font-bold uppercase tracking-wider text-text-muted mb-1.5 pl-1">Receiver Name</label>
                        <input 
                          required
                          placeholder="John Doe"
                          className="w-full px-4 py-3 bg-bg-base border border-border-base rounded-xl sm:rounded-2xl outline-none focus:bg-white focus:border-primary transition-all text-sm"
                          value={checkoutData.name}
                          onChange={(e) => setCheckoutData({...checkoutData, name: e.target.value})}
                        />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider text-text-muted mb-1.5 pl-1">Mobile Phone</label>
                        <input 
                          required type="tel"
                      ...
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider text-text-muted mb-1.5 pl-1">Email (Optional)</label>
                        <input 
                          type="email"
                      ...
                      {isBuyNowFlow && selectedProduct && (
                        <>
                          <div>
                            <label className="block text-[10px] font-bold uppercase tracking-wider text-text-muted mb-1.5 pl-1">Size</label>
                            <select 
                              className="w-full px-4 py-3 bg-bg-base border border-border-base rounded-xl sm:rounded-2xl outline-none focus:bg-white focus:border-primary transition-all appearance-none cursor-pointer text-sm"
                          ...
                          <div>
                            <label className="block text-[10px] font-bold uppercase tracking-wider text-text-muted mb-1.5 pl-1">Color</label>
                            <select 
                              className="w-full px-4 py-3 bg-bg-base border border-border-base rounded-xl sm:rounded-2xl outline-none focus:bg-white focus:border-primary transition-all appearance-none cursor-pointer text-sm"
                      ...
                      <div className="sm:col-span-2">
                        <label className="block text-[10px] font-bold uppercase tracking-wider text-text-muted mb-1.5 pl-1">Delivery Address</label>
                        <textarea 
                          required rows={2}
                          placeholder="House No, Area, City..."
                          className="w-full px-4 py-3 bg-bg-base border border-border-base rounded-xl sm:rounded-2xl outline-none focus:bg-white focus:border-primary transition-all resize-none text-sm"
                          value={checkoutData.address}
                          onChange={(e) => setCheckoutData({...checkoutData, address: e.target.value})}
                        />
                      </div>
                      <div className="sm:col-span-2">
                        <label className="block text-[10px] font-bold uppercase tracking-wider text-text-muted mb-1.5 pl-1">Order Note (Optional)</label>
                        <textarea 
                          rows={1}
                    ...
                    <div className="space-y-3 pt-2">
                      <label className="block text-[10px] font-bold uppercase tracking-wider text-text-muted mb-1 pl-1">Payment Method</label>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        {[
                          ...
                          <button
                            ...
                            className={`flex items-center gap-3 px-3 py-3 rounded-xl font-bold text-[11px] transition-all border ${checkoutData.paymentMethod === method.id ? 'bg-primary text-white border-primary shadow-lg shadow-indigo-100 scale-[1.02]' : 'bg-white text-text-muted border-border-base hover:bg-bg-base'}`}
                          >
                            <method.icon size={14} />
                            {method.id}
                          </button>
                    ...
                    <div className="pt-4 pb-2">
                      <button 
                        ...
                        className={`w-full bg-primary text-white py-4 sm:py-5 rounded-xl sm:rounded-2xl font-black text-lg sm:text-xl transition-all shadow-xl shadow-indigo-200 flex items-center justify-center gap-3 group ${isSubmittingOrder ? 'opacity-70 cursor-not-allowed' : 'hover:bg-primary-hover active:scale-[0.98]'}`}
                        id="confirm-order-button"
                      >
                        {isSubmittingOrder ? (
                          <>Processing Order...</>
                        ) : (
                          <>Confirm My Order <Check size={20} className="group-hover:scale-125 transition-transform" /></>
                        )}
                      </button>
                      <p className="text-center text-[9px] text-text-muted font-bold uppercase tracking-tight mt-4">
        ...
        {isOrderSuccess && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              className="relative bg-white w-full max-w-sm rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 text-center shadow-2xl"
            >
              <div className="w-16 h-16 sm:w-20 sm:h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-6 scale-110 sm:scale-125">
                <Check size={32} />
              </div>
              <h3 className="text-2xl sm:text-3xl font-black mb-2 text-text-base">Order Confirmed!</h3>
              ...
              <button 
                onClick={() => setIsOrderSuccess(false)}
                className="w-full bg-primary text-white py-3 sm:py-4 rounded-xl font-bold shadow-lg shadow-indigo-100 hover:shadow-xl transition-all text-sm sm:text-base"
              >
                Continue Shopping
              </button>
            </motion.div>
